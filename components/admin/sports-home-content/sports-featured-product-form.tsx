...
export default function SportsFeaturedProductForm({ products, mode = "create", item }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url =
        mode === "edit"
          ? `/api/admin/sports-featured-products/${item?.id}`
          : "/api/admin/sports-featured-products";
  ...
  return (
    <div className="p-6 max-w-2xl">
      ...
      <h1 className="text-2xl font-bold text-white mb-6">
        {mode === "edit" ? "Edit" : "Add"} Featured Gear Product
      </h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-[#111827] p-6 rounded-xl border border-[#1E293B]">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-600">
            Shown in the &quot;Featured Gear&quot; grid on the sports homepage. Only the first 200 products are listed.
          </p>
        </div>

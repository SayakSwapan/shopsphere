...
import { RETURN_FLOW_GUIDE } from "@/lib/flow-guides";
...
export default async function AdminReturnDetailPage({ params }: Props) {
  ...
  const req = await prisma.return_request.findUnique({
    where: { id },
    include: {
      user: {
    ...
    <PageContainer>
      ...
      <div className="mb-6 rounded-2xl border border-slate-700 bg-[#111827] p-5">
        ...
        <ReturnStatusActions
          ...
          defaultRefundAmount={Number(order.totalAmount)}
        />
      </div>

      {/* Guide */}
      <div className="mb-6">
        <FlowGuide steps={RETURN_FLOW_GUIDE} currentStatus={req.status} />
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-slate-700 bg-[#111827] p-6">
            <div className="mb-4 flex items-center gap-3">
              ...
              <h2 className="text-lg font-bold text-white">Pickup & Refund</h2>
            </div>
            <dl className="space-y-2 text-sm">
              ...
              {req.refundAmount != null && (
                <div className="flex justify-between gap-4">
                  ...
                  <dd className="font-bold text-emerald-400">{formatCurrency(Number(req.refundAmount))}</dd>
                </div>
              )}
              {req.refundMethod && (
                <div className="flex justify-between gap-4">
              ...
              {req.refundProcessedAt && (
                <div className="flex justify-between gap-4">
                  <dt className="text-slate-500">Refund processed</dt>
                  <dd className="text-slate-300">{formatDateTime(req.refundProcessedAt)}</dd>
                </div>
              )}
            </dl>
          </div>
          ...
          <div className="rounded-2xl border border-slate-700 bg-[#111827] p-6">
            <div className="mb-4 flex items-center gap-3">
              ...
              <h2 className="text-lg font-bold text-white">Refund Bank Details</h2>
            </div>
            {bankDetails ? (
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between gap-4">
                  <dt className="text-slate-500">Account holder</dt>
                  <dd className="text-right font-semibold text-white">{bankDetails.accountHolder}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-slate-500">Account number</dt>
                  <dd className="font-mono text-xs text-slate-300">{maskAccountNumber(bankDetails.accountNumber)}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-slate-500">Bank</dt>
                  <dd className="text-right text-slate-300">{bankDetails.bankName}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-slate-500">Branch</dt>
                  <dd className="text-right text-slate-300">{bankDetails.branchName}</dd>
                </div>
                <div className="flex justify-between gap-4">
              ...
              <p className="text-sm text-slate-500">
                Customer has not submitted bank details yet. Refund can only be initiated once
                these are on file.
              </p>
            )}
          </div>

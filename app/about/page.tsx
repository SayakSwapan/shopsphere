...
export default async function AboutPage() {
  ...
  const heading = s.about_heading || "About TrinovaSports";
  const aboutHtml = s.about_text || "";
  const siteName = s.site_name || SITE_NAME_FALLBACK;
  const tagline =
    s.footer_tagline ||
  ...
  const values = [
    {
      icon: ShieldCheck,
      title: "Trust & Quality",
      text: "Every product is verified for authenticity and quality before it reaches you.",
    },
    {
      icon: Truck,
      title: "Fast Delivery",
      text: "Pan-India delivery with real-time tracking on every order.",
    },
    {
      icon: RotateCcw,
      title: "Easy Returns",
    ...
    },
    {
      ...
      text: "Dedicated support team committed to resolving every query with care.",
    },
  ];
    ...
    <div className="min-h-screen bg-bg-page">
      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-24 text-center">
          ...
          <div
            className="mt-4 text-sm max-w-md mx-auto leading-relaxed text-text-muted-1"
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
          ...
          <div className="mb-16">
            <div className="rounded-4xl border border-border-card bg-bg-card p-8 sm:p-12 text-center">
              <p className="text-text-muted-1">
                Welcome to {siteName}! We are building something special. Stay
                tuned for our story.
              </p>
            </div>
          </div>
        ...
        <div>
          <div className="text-center mb-12">
            ...
            <h2
              className="text-3xl sm:text-4xl font-extrabold uppercase leading-none text-text-heading tracking-tight"
              style={{ fontFamily: "var(--t-font-heading)" }}
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {values.map((item) => (
              ...
              >
                ...
                <h3 className="text-sm font-bold uppercase tracking-wider text-text-heading mb-2">
                  {item.title}
                </h3>
                <p className="text-xs text-text-muted-1 leading-relaxed">
                  {item.text}
                </p>
              </div>
            ))}

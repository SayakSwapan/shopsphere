...
export default function EditStatCounterPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/stat-counters/${id}`)
      .then((r) => r.json())
      .then((data) => {
        setForm({
          value: data.value || "",
      ...
      .catch(() => toast.error("Failed to load stat counter"))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/admin/stat-counters/${id}`, {
        method: "PUT",
      ...
      router.push("/admin/stat-counters");
    } catch (err) {
  ...
  return (
    <div className="p-6 max-w-2xl">
      <Link href="/admin/stat-counters" className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-white mb-4 transition-colors">
        <ArrowLeft size={16} />
        Back to Stat Counters
      </Link>

      <h1 className="text-2xl font-bold text-white mb-6">Edit Stat Counter</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-[#111827] p-6 rounded-xl border border-[#1E293B]">
        ...
        <div className="flex justify-end gap-3 pt-4">
          <Link href="/admin/stat-counters" className="px-4 py-2 text-sm text-slate-400 hover:text-white">Cancel</Link>

...
function getActiveSection(pathname: string): string | null {
  const activeSection = navItems
    .filter(
      (e): e is Extract<NavEntry, { type: "section" }> => e.type === "section",
    )
    .find((s) =>
      s.children.some(
...
export default function AdminSidebar({
  ...
  // Raw value — when the admin blanks the site name we keep it blank instead
  // of falling back to a brand default (the name now lives on the logo itself).
      ...
      return (
        <Link
          ...
          style={{
            ...
            boxShadow: active
              ? "inset 3px 0 0 #F59E0B, 0 2px 12px rgba(245,158,11,0.12)"
              : "none",
  ...
  return (
    <aside
      ...
      style={{
        background:
          "linear-gradient(180deg, #0B1220 0%, #0E1626 55%, #0B1220 100%)",
    ...
    >
      ...
      >
        {siteLogo ? (
          ...
          >
            <Store size={20} className="text-[#0A0F1E]" strokeWidth={2.4} />
          </div>
        )}
        <div className="min-w-0 leading-tight">
          {rawSiteName && !siteLogo && (
      ...
      >
        {rawSiteName ? `${rawSiteName} · v1 · Private` : "v1 · Private"}
      </div>
    </aside>
  );
}

...
export default function TemplateForm({ initialData }: Props) {
  ...
  function insertPlaceholder(ph: string) {
    ...
    if (subjectEl && document.activeElement === subjectEl) {
      ...
      const newSubject = form.subject.substring(0, start!) + ph + form.subject.substring(end!);
      setForm((prev) => ({ ...prev, subject: newSubject }));
      setTimeout(() => {
        subjectEl.focus();
        subjectEl.selectionStart = start! + ph.length;
        subjectEl.selectionEnd = start! + ph.length;
      }, 0);
    } else {
  ...
  async function handleSubmit() {
    if (!form.templateKey || !form.templateName || !form.subject || !form.body) {
    ...
    try {
      const res = await fetch(
        initialData ? `/api/admin/email-templates/${initialData.id}` : "/api/admin/email-templates",
        {
    ...
    <div className="max-w-5xl space-y-6">
      <div className="rounded-2xl border border-slate-800 bg-[#111827] p-8">
        <h2 className="mb-8 text-2xl font-bold text-white">
          {initialData ? "Edit" : "Create"} Email Template
        </h2>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Login OTP Email"
              />
            </div>
          </div>
          ...
          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-300">Email Subject *</label>
            <input
              ref={subjectRef}
              type="text"
              value={form.subject}
              ...
              className="w-full rounded-xl border border-slate-700 bg-[#0F172A] px-4 py-3 text-white text-sm outline-none focus:border-amber-500"
              placeholder="e.g. Your OTP Code is {{otp}}"
            />
          </div>

          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-300">Email Body *</label>
            <EmailBodyEditor
              value={form.body}
          ...
          <div>
            ...
            />
            <p className="mt-1 text-xs text-slate-500">Click a placeholder below to insert it into the subject or body</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {COMMON_PLACEHOLDERS.map((ph) => (
                <button
                  ...
                  className="rounded-lg bg-slate-700/50 px-2.5 py-1 text-xs text-slate-300 hover:bg-amber-500/20 hover:text-amber-400 transition"
                >
                  {ph}
                </button>
              ))}

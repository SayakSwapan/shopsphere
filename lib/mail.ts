...
export async function sendOtpEmail(email: string, otp: string) {
  await sendFromTemplate(
    ...
    `<div style="background:#0A0F1E;color:white;padding:40px;font-family:Arial;"><h1 style="color:#F5A623;">{{siteName}}</h1><p>Your OTP is:</p><h2 style="letter-spacing:8px;color:#F5A623;">${escapeEmailHtml(otp)}</h2><p>Valid for 10 minutes.</p></div>`
  );
}
...
export async function sendPasswordResetEmail(email: string, otp: string, userName: string | null) {
  ...
  await sendFromTemplate(
    ...
    `<div style="background:#0A0F1E;color:white;padding:48px 40px;font-family:Arial;"><h1 style="color:#F5A623;">{{siteName}}</h1><p>Hello ${escapeEmailHtml(name)},</p><p>Your password reset OTP is:</p><h2 style="letter-spacing:8px;color:#F5A623;">${escapeEmailHtml(otp)}</h2><p>Valid for 10 minutes.</p></div>`
  );
}
  ...
  try {
    ...
    await emailTransporter.sendMail({
      ...
      <div style="background:#0A0F1E;color:#ffffff;padding:48px 40px;font-family:Arial,Helvetica,sans-serif;">
        <h1 style="color:#F5A623;font-size:28px;margin:0 0 8px 0;">${escapeEmailHtml(siteName)}</h1>
        ...
        <div style="background:#111827;border-radius:16px;padding:36px 32px;">
          ...
          <h2 style="color:#ffffff;font-size:22px;margin:0 0 8px 0;">Hello, ${escapeEmailHtml(customerName)}</h2>
          ...
          <div style="margin-bottom:24px;">
            ...
            <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:20px;">
              <p style="color:#F5A623;font-size:12px;font-weight:bold;margin:0 0 6px 0;">Subject: ${escapeEmailHtml(subject)}</p>
              <p style="color:#CBD5E1;font-size:14px;line-height:1.7;margin:0;white-space:pre-wrap;">${escapeEmailHtml(customerMessage)}</p>
            </div>
          </div>
          <div style="margin-bottom:24px;">
            ...
            <div style="background:rgba(245,166,35,0.06);border:1px solid rgba(245,166,35,0.15);border-radius:12px;padding:20px;">
              <p style="color:#CBD5E1;font-size:14px;line-height:1.7;margin:0;white-space:pre-wrap;">${escapeEmailHtml(adminReply)}</p>
            </div>
          </div>
        </div>
        <p style="color:#3A4455;font-size:11px;margin-top:32px;text-align:center;">&copy; ${new Date().getFullYear()} ${escapeEmailHtml(siteName)}. All rights reserved.</p>
      </div>
    `,
    });
  } catch (error) {
...
export async function sendPasswordResetSuccessEmail(email: string, userName: string | null) {
  ...
  await sendFromTemplate(
    ...
    `<div style="background:#0A0F1E;color:white;padding:48px 40px;font-family:Arial;"><h1 style="color:#F5A623;">{{siteName}}</h1><p>Hello ${escapeEmailHtml(name)},</p><p>Your password has been successfully changed.</p></div>`
  );
}
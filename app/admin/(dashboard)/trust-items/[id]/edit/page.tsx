...
export default function EditTrustItemPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/trust-items/${id}`)
      .then((r) => r.json())
      .then((data) => {
        setForm({
          icon: data.icon || "",
      ...
      .catch(() => toast.error("Failed to load trust item"))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/admin/trust-items/${id}`, {
        method: "PUT",
  ...
  return (
    <div className="p-6 max-w-2xl">
      ...
      <h1 className="text-2xl font-bold text-white mb-6">Edit Trust Item</h1>

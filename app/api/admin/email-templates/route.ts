...
export async function POST(req: NextRequest) {
  ...
  const { templateKey, templateName, subject, body, description, placeholders, isActive } = await req.json();

  if (!templateKey || !templateName || !subject || !body) {
    return NextResponse.json({ message: "Template key, name, subject and body are required." }, { status: 400 });
  }

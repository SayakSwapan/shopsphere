  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
        <div>
          <h1 className="text-4xl font-black tracking-tight">WhatsApp Templates</h1>
          ...
          <table className="w-full">
            <thead style={{ background: "#0F172A" }}>
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-slate-400">Preview</th>
                ...
                filtered.map((t) => (
                  <tr key={t.id} className="border-t border-slate-700 hover:bg-slate-800/40 transition">
                    <td className="px-6 py-5">
                      ...
                      <p className="text-xs text-green-400 font-mono mt-1">{t.templateKey}</p>
                      {t.description && <p className="text-xs text-slate-500 mt-1">{t.description}</p>}
                    </td>
                    <td className="px-6 py-5 text-sm text-slate-300 max-w-[300px] truncate">{t.body}</td>
                    <td className="px-6 py-5">
                      <button
                    ...
                    <td className="px-6 py-5">
                      <div className="flex gap-2">
                        <button
                          onClick={() => router.push(`/admin/whatsapp-templates/${t.id}`)}

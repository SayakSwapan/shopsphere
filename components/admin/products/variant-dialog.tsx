...
interface Props {
  ...
  setEditingVariant: React.Dispatch<React.SetStateAction<VariantType | null>>;
...
export default function VariantDialog({
  ...
  const [previousVariantId, setPreviousVariantId] = useState<string | null>(
    null,
  );

  if (open && previousVariantId !== (editingVariant?.id ?? null)) {
  ...
  const usedStock = variants.reduce((total, variant) => {
    if (editingVariant && variant.id === editingVariant.id) {
      return total;
    }
  ...
  const remainingStock = productStock - usedStock;
  ...
  function saveVariant() {
    ...
    const allowedStock = remainingStock + (editingVariant?.stock ?? 0);

    if (form.stock > allowedStock) {
      alert(`Only ${allowedStock} stock available.`);

      return;
    }

    const gender = genders.find((g) => g.id === form.genderId);

    const size = sizes.find((s) => s.id === form.sizeId);

    const variant: VariantType = {
      id: editingVariant?.id ?? `${Date.now()}-${Math.random()}`,
    ...
    if (editingVariant) {
      setVariants((prev) =>
        prev.map((v) => (v.id === editingVariant.id ? variant : v)),
      );
    } else {
      ...
      <div className="flex max-h-[92dvh] w-full max-w-[650px] flex-col overflow-hidden rounded-2xl bg-[#111827] shadow-2xl">
        ...
        <div className="overflow-y-auto overscroll-contain px-6 py-4 sm:px-7">
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            {isFreeSize ? (
              <div className="col-span-2">
                <label className="mb-2 block text-sm text-white">Size</label>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  This category uses a single free size. Gender and size are
                  pre-selected.
                </p>
              </div>
            ) : (
              <>
                ...
                <div>
                  ...
                  >
                    ...
                    {sizes
                      .filter((size) => size.genderId === form.genderId)
                      .map((size) => (
                        <option
                          key={size.id}
                          value={size.id}
          ...
          <div className="mt-6 rounded-xl bg-[#0F172A] p-4 text-sm text-amber-400">
            Remaining Product Stock : {remainingStock}
          </div>
          ...
          <div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <button
              type="button"
              onClick={closeDialog}
              className="w-full rounded-xl border border-slate-700 px-6 py-3 text-white sm:w-auto"
            >
              Cancel
            </button>
            ...
            <button
              type="button"
              onClick={saveVariant}
              className="w-full rounded-xl bg-amber-500 px-8 py-3 font-bold text-black hover:bg-amber-400 sm:w-auto"
            >
